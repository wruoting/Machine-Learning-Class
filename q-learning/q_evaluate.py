...
def q_evaluate():
    ...
    df_bullish = csv_to_dataframe('sin-real.csv')
    plot_name = 'Sine-Real.html'
    model = load_model('q_model_sine.h5')
    stored_buffer = 50
...
def evaluate_q_epoch(state, price_data, model, decision_state, batch_range, stored_buffer):
    ...
    while not terminal_state:
        ...
        action_to_reward = get_reward(state_index, action, price_data, decision_state, batch_size)
